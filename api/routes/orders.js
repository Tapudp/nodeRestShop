...
router.get("/", checkAuth,  checkAuth,  (req, res, next) => {
  Order.find()
    ...
    .then(docs => {
      res.status(200).json({
        "total number of orders are ": docs.length,
        orders: docs.map(doc => {
          return {
            _id: doc._id,
            product: doc.product,
  ...
  Product.findById(req.body.productId)
    .then(product => {
      ...
      const order = new Order({
        _id: mongoose.Types.ObjectId(), // execute as an function to automatically generate an id
...
router.get("/:orderId", checkAuth,  (req, res, next) => {
  Order.findById(req.params.orderId)
    ...
    .then(order => {
      if(!order){
        return res.status(404).json({
          message: "Order not found આવું કઈ નથી કા તો ડિલીટ થઈ ગયું "
        })
      }
      res.status(200).json({
        order: order,
